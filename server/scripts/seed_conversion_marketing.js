...
const REVIEW_COMMENTS = [
    "Melhor corte da região, recomendo demais!",
    "Atendimento excelente, o Marcelo é muito detalhista.",
    "Ambiente top e profissionais nota 10.",
    "Barba perfeita, produtos de qualidade.",
    "Sempre saio satisfeito, Rafael manda muito bem.",
    "App de agendamento facilita muito a vida.",
    "O melhor degradê que já fiz, parabéns!",
    "Café bom e corte melhor ainda.",
    "Pontualidade e profissionalismo impecáveis.",
    "Já sou cliente fiel, não troco por nada."
];
...
async function wipeDemoData() {
    ...
    if (demoShop) {
        ...
        await prisma.user.updateMany({ where: { workedBarbershopId: demoShop.id }, data: { workedBarbershopId: null } });
...
async function main() {
    ...
    console.log('Activating Loyalty Program...');
    await prisma.loyaltyProgram.upsert({
        where: { barbershopId: shop.id },
        update: { active: true, rewardDescription: 'Corte Grátis após 10 visitas', minPointsToRedeem: 100 },
        create: { barbershopId: shop.id, active: true, rewardDescription: 'Corte Grátis após 10 visitas', minPointsToRedeem: 100 }
    });

    console.log('Creating 150 unique clients...');
    ...
    for (let i = 0; i < TARGET_COUNT; i++) {
        ...
        if (i < hardcodedSlots.length) {
            ...
            const [sh, sm] = slotData.time.split(':'); h = parseInt(sh); m = parseInt(sm);
        } else {
            ...
            m += 15; if (m >= 60) { m -= 60; h++; } if (h > 19) h = 9;
        }
        ...
        await prisma.commission.create({
            data: {
                barberId: slotData.barber.id, barbershopId: shop.id, appointmentId: appt.id,
                type: 'SERVICE', amount: Number(slotData.service.price) * 0.5, percentage: 50,
        ...
        await prisma.transaction.create({
            data: {
                description: `Serviço: ${slotData.service.name}`, amount: slotData.service.price,
                type: 'INCOME', date, barbershopId: shop.id, appointmentId: appt.id,
                professionalId: slotData.barber.id, paymentMethod: 'PIX'
            }
        });
    }

    // --- HISTORICAL DATA (30 DAYS) ---
    console.log('Generating 30 days of historical data + Reviews...');
    for (let d = 30; d >= 1; d--) {
        ...
        for (let i = 0; i < count; i++) {
            ...
            const appt = await prisma.appointment.create({
                data: {
                    date: apptDate, status: 'COMPLETED',
                    clientId: client.id, professionalId: barber.id,
                    serviceId: srv.id, barbershopId: shop.id, paymentStatus: 'PAID'
                }
            });

            await prisma.transaction.create({
                data: {
                    description: `Serviço: ${srv.name}`, amount: srv.price,
                    type: 'INCOME', date: apptDate, barbershopId: shop.id,
                    appointmentId: appt.id, professionalId: barber.id
                }
            });

            // 15% chance of a high-conversion review
            if (Math.random() < 0.15) {
        ...
        // Add Categorized Expenses (Conversion factor: transparency/professionalism)
        const dayExpenses = [
            { desc: 'Marketing Redes Sociais', amount: 50, category: 'Marketing' },
            { desc: 'Limpeza e Higiene', amount: 30, category: 'Manutenção' },
            { desc: 'Insumos (Gola, Lâmina)', amount: 45, category: 'Produtos' }
        ];
        for (const exp of dayExpenses) {
            await prisma.transaction.create({
                data: {
                    description: exp.desc, amount: exp.amount,
                    type: 'EXPENSE', category: exp.category,
                    date: curDate, barbershopId: shop.id
                }
            });
        }
    }

    // --- WAITLIST (High Demand Social Proof) ---
    console.log('Generating Waitlist entries...');
    const waitlistData = ["Mariano", "Gustavo", "Fernando", "Ricardo", "Samuel"];
    for (const name of waitlistData) {
        await prisma.waitlist.create({
            data: {
                clientName: name,
                clientPhone: `119${Math.floor(10000000 + Math.random() * 89999999)}`,
    ...
    console.log('CONVERSION SEED COMPLETED!');
}

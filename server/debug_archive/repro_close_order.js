...
async function main() {
    try {
        ...
        if (!shop) {
            console.log('Shops found but none with staff/services:');
            shops.forEach(s => console.log(`- ${s.name}: Staff=${s.staff.length}, Services=${s.services.length}`));
        ...
        const result = await prisma.$transaction(async (tx) => {
            const transaction = await TransactionService.createTransaction({
                ...
                method: 'PIX', // Valid Enum String? Check schema.
                origin: 'PRESENCIAL',
                orderId: order.id,
                professionalId: pro.id,
